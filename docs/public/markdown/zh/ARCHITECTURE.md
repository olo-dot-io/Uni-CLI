<!-- 由 docs/zh/ARCHITECTURE.md 生成。不要直接编辑此副本。 -->

# 架构

- 规范页: https://olo-dot-io.github.io/Uni-CLI/zh/ARCHITECTURE
- Markdown: https://olo-dot-io.github.io/Uni-CLI/markdown/zh/ARCHITECTURE.md
- 栏目: 解释
...
Uni-CLI 的长期目标不是再做一个协议外壳，而是成为智能体控制网站、桌面应用、本地工具和文件的控制内核。内核要保持小、可审计、可验证：
...
外部趋势不能替代代码、测试和 git 证据，但可以更新方向判断。近期趋势对 Uni-CLI 的启发是：
...
这些趋势反而强化了本地架构：命令优先、manifest 可信、adapter 可修复、权限显式、传输多元。

## 行业位置

Uni-CLI 位于 agent 应用之下、真实网站/桌面应用/本地工具之上。它不是 IDE、不是聊天产品、不是模型托管层，也不是单一 agent loop；它是这些产品都应该能调用的执行底座。
