...
- `approval_memory`：稳定的命令 scope key 和 capability 维度摘要。
  `--remember-approval` 会把允许的 scope 存到 `~/.unicli/approvals.jsonl`，
  文件只含 scope metadata。`unicli approvals list`、`revoke`、`clear` 用来查看或移除已记住的 scope。
