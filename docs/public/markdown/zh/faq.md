...
Uni-CLI 是给 AI Agent 用的命令行执行层。它把网站、桌面应用、MCP 服务和外部 CLI 编译成一份可搜索的命令目录，所有调用都走同一条命令路径，返回稳定的 v2 AgentEnvelope。当前覆盖 235 个站点、1450 条命令。
...
v0.217 覆盖 235 个站点、1450 条命令、1040 个适配器、59 个 pipeline step、7591 个测试。社交平台、开发者工具、中文站点、学术数据库、政府政策、播客、macOS 应用都覆盖了。
