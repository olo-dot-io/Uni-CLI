...
| `quarantine`         | Skip this command in CI until a maintainer verifies and explicitly restores it. `unicli repair` reports but never changes this field. | `false`.                                                                                                                |
...
| `HTTP 403 Forbidden` on cookie strategy                   | Authentication is stale or insufficient                                     | `unicli auth setup SITE`, then rerun the original command; do not edit the adapter unless fresh auth disproves this diagnosis. |
...
| `visual_backend: remote not configured`                   | Visual backend endpoint or API key is missing                               | Configure the backend for explicitly selected visual operations, or use a structured route.                                    |
| `quarantine: true` blocks execution                       | The adapter is intentionally gated                                          | Inspect the recorded reason, repair the exact source, verify with `unicli repair SITE CMD`, then explicitly remove the flag.   |
