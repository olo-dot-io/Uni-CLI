# Uni-CLI Roadmap — Mission Vostok (0.200)

> Target: Full coverage of OpenCLI (73 sites, 460+ commands) + CLI-Anything (32 desktop adapters) + AI self-healing engine + our unique additions.

## The Differentiator: AI Self-Healing

OpenCLI's creator said it best:

> "本质上就是不够 native。整个过程中 AI 不方便自修复，还得让用户手动提 PR 才能修复自己。如果能够让 AI 很轻易的有修复网站适配器甚至是 core 的能力，就不会有任何问题了。"

Uni-CLI solves this: 20-line YAML adapters are trivially readable, editable, and testable by any AI agent. No build step, no Python env, no npm link. Edit YAML → re-run → fixed.
...
### Phase 0: Self-Healing Engine (before any new adapters)

- [ ] Structured error JSON from pipeline failures
- [ ] `unicli repair <site> <command>` diagnostic
- [ ] User adapter overlay: `~/.unicli/adapters/` overrides built-in
- [ ] `unicli test <site>` health check for all commands
- [ ] Desktop adapter subprocess executor (`type: desktop`)
- [ ] Bridge adapter passthrough executor (`type: bridge`)

### Phase 1: Public API Adapters (22 sites, pure YAML)

Priority order by daily usefulness:
1. Complete hackernews (6 remaining: best, new, show, ask, jobs, user)
2. Complete reddit (13 remaining: frontpage, popular, subreddit, etc.)
3. lobsters, stackoverflow, bbc, wikipedia
4. arxiv, devto, hf, imdb, bluesky
5. google, spotify, steam, dictionary
6. apple-podcasts, ctrip, xiaoyuzhou, bloomberg
7. v2ex, sinafinance, paperreview, chatgpt, web
...
## Non-Adapter Work for 0.200

- [ ] Browser extension (Chrome MV3) for cookie strategy
- [ ] Daemon process for browser bridge
- [ ] MCP server implementation (not just stub)
- [ ] Plugin system: `unicli plugin install github:user/repo`
- [ ] `unicli explore <url>` — API discovery
- [ ] `unicli synthesize <site>` — adapter generation
- [ ] `unicli operate` — browser automation commands
