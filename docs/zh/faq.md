...
Uni-CLI 是给 AI Agent 用的命令行执行层。它把网站、桌面应用、MCP 服务和外部 CLI 编译成一份可搜索的命令目录，所有调用都走同一条命令路径，返回稳定的 v2 AgentEnvelope。当前覆盖 <span><!-- STATS:site_count -->282<!-- /STATS --></span> 个站点、<span><!-- STATS:command_count -->1692<!-- /STATS --></span> 条命令。
