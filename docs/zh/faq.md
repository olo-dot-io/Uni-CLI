...
v1.0.4 的静态 adapter 目录包含 <span><!-- STATS:site_count -->337<!-- /STATS --></span> 个站点。

同一目录包含 <span><!-- STATS:command_count -->1884<!-- /STATS --></span> 条注册命令。Core command 与主机发现的工具会在运行时加入。可以浏览[操作目录](/zh/reference/sites)，也可以运行 `unicli list`。
