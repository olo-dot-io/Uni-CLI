...
每条命令都返回 v2 AgentEnvelope——成功失败同一个形状。Agent 用一份 schema 解析 <span><!-- STATS:command_count -->1692<!-- /STATS --></span> 条命令。
...
这就是整套交互模型。一种命令形状跨 <span><!-- STATS:site_count -->282<!-- /STATS --></span> 个站点、<span><!-- STATS:command_count -->1692<!-- /STATS --></span> 条命令。一种错误回执跨每一次失败。一条 self-repair 路径跨每一个适配器。
