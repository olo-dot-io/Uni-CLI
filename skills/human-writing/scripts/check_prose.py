#!/usr/bin/env python3
"""检查中文和英文成稿的硬禁令与常见模型化形状。只报警，不自动改文。"""
...
PIVOT_PATTERNS = (
    ...
    re.compile(r"看似[^。！？\n]{0,90}(?:其实|实际|实则)"),
    re.compile(
        r"\b(?:is|are|was|were|do|does|did|can|could|should|would|will|has|have|had)\s+not\b[^.!?\n]{0,140}\bbut\b",
        re.IGNORECASE,
    ),
    re.compile(
        r"\b(?:isn't|aren't|wasn't|weren't|don't|doesn't|didn't|can't|couldn't|shouldn't|wouldn't|won't|hasn't|haven't|hadn't)\b[^.!?\n]{0,140}\bbut\b",
        re.IGNORECASE,
    ),
...
def main() -> int:
    parser = argparse.ArgumentParser(description="检查中文和英文成稿的硬禁令与模型化形状")
    ...
    total_latin = latin_word_count(prose)
    if total_han == 0 and total_latin == 0:
        print("没有检测到可检查的正文。", file=sys.stderr)
        return 2
    ...
    print(f"汉字数 {total_han}，英文词数 {total_latin}")
    print(
