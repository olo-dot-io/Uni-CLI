...
export function flattenXhsNoteGroups(noteGroups: unknown): unknown[] {
...
export function buildXhsNoteUrl(
  userId: string,
  noteId: string,
  xsecToken?: string,
): string {
...
export function extractXhsUserNotes(
  ...
  for (const entry of notes) {
    ...
    const noteCard = e?.noteCard ?? e?.note_card ?? e;
    ...
    const userId = toCleanString(
      noteCard.user?.userId ?? noteCard.user?.user_id ?? fallbackUserId,
    );
    ...
    rows.push({
      id: noteId,
      title: toCleanString(
        noteCard.displayTitle ?? noteCard.display_title ?? noteCard.title,
      ),
