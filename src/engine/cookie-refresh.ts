 ...
 * @does        Refreshes a site's login session by navigating a broker-owned browser target and re-reading cookies, surfacing the real cause of a miss
 *              (no browser / no cookies / error) instead of a bare boolean.
 * @needs       ../browser/bridge (lazy), ambient browser invocation scope, ./cookies process-memory handoff
 * @feeds       src::engine::runtime (maybeRefreshCookies — the auto 401/403 path)
 * @breaks      Browser/navigation failures become a typed SessionRefreshOutcome; request cancellation throws its AbortSignal reason after cleanup.
 ...
 * @side-effects Acquires a broker-owned browser target, navigates it, and retains refreshed cookies briefly in process memory; never persists.
...
export interface SessionRefreshDeps {
  connect(signal?: AbortSignal): Promise<PageSession>;
}
...
async function defaultConnect(): Promise<PageSession> {
...
export async function refreshCookies(
  ...
  try {
    page = await deps.connect(signal);
  } catch (err) {
    signal?.throwIfAborted();
    return {
      status: "no-browser",
      detail: `Could not acquire a broker-owned browser target: ${
  ...
  try {
    ...
    if (signal?.aborted) {
      outcome = { status: "error", detail: "Cookie refresh was cancelled" };
    } else {
      outcome = {
        status: "error",
