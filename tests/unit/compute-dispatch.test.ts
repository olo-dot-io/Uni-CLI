...
} from "../../src/transport/compute-dispatch.js";
import { planComputeRoute } from "../../src/transport/routing.js";
import type {
  ActionRequest,
  ActionResult,
  Capability,
...
describe("compute dispatch", () => {
  it("retries one transient read through the same provider and physical primitive", async () => {
    ...
    uia.snapshotFactory = () => {
      snapshotAttempt++;
      if (snapshotAttempt === 1) throw new Error("sidecar exited with code 7");
      return {
        format: "json",
        data: JSON.stringify({
          role: "Window",
    ...
    const result = await dispatchComputeRoute(
      bus,
      {
        ...
        params: { app: "Calculator", windowId: "0x2" },
      },
      "win32",
    );

    expect(result.ok).toBe(true);
    expect(uia.snapshots).toHaveLength(2);
    expect(uia.recoverCount).toBe(1);
    expect(result.recovery_trace).toMatchObject({
      strategy: "same-primitive-retry",
      attempts: 2,
      recovered: true,
      provider: "desktop-uia",
      physical_action: "uia_snapshot",
      failures: [
        {
          attempt: 1,
          trigger: "retryable-read-failure",
          reason: "sidecar exited with code 7",
        },
      ],
    });
  });

  it("never automatically retries a mutation even when its envelope is retryable", async () => {
    const bus = createTransportBus();
    const uia = new StubTransport(
      "desktop-uia",
      ["uia_invoke"],
      err({
        ...
        step: 0,
        action: "uia_invoke",
        reason: "transient provider failure after dispatch",
        suggestion: "inspect target state",
        retryable: true,
        exit_code: 75,
      }),
    );
    ...
    expect(uia.calls).toHaveLength(1);
    expect(uia.recoverCount).toBe(0);
    expect(result.recovery_trace).toMatchObject({
      strategy: "none",
      attempts: 1,
      recovered: false,
      provider: "desktop-uia",
      physical_action: "uia_invoke",
    });
  ...
  it("never replays a structurally ambiguous CDP mutation on a fallback transport", async () => {
    ...
    // REASON: the CDP page is the external browser boundary; the real adapter, bus, and selected-provider dispatch remain integrated.
  ...
  it("marks cancellation of an unsettled mutation ambiguous and never falls through", async () => {
    ...
    bus.register(cdp);
    bus.register(visual);
    const execution = dispatchComputeRoute(
      bus,
      {
        kind: "compute_click",
        params: {
  ...
  it("selects one platform-native provider and keeps visual explicit", () => {
    for (const [platform, transport] of [
      ["darwin", "desktop-ax"],
      ["win32", "desktop-uia"],
      ["linux", "desktop-atspi"],
    ] as const) {
      ...
      if (platform === "darwin") {
        expect(decision).toMatchObject({
          status: "unavailable",
          reason: expect.stringContaining("requires an exact app"),
        });
      } else {
        expect(decision).toMatchObject({
          status: "selected",
      ...
      expect(
        decision.candidates.find(
          (candidate) => candidate.transport === "visual",
        ),
      ).toBeUndefined();
    }
  });
  ...
  it("uses CDP screenshots only when the browser route is explicit", async () => {
    const bus = createTransportBus();
    ...
    const result = await dispatchComputeRoute(
      bus,
      { kind: "compute_screenshot", params: { via: "browser" } },
      "darwin",
    );
  ...
  it("selects macOS AX screenshots without opening the visual provider", async () => {
    const bus = createTransportBus();
    const ax = new StubTransport(
      "desktop-ax",
      ["ax_screenshot"],
  ...
  it("evaluates compute_assert locally from current provider-owned refs", async () => {
    ...
    winBus.register(uia);
    const winRefs = new RefAllocator();
    winRefs.alloc({
      stable: "desktop-uia:settings:title",
      role: "text",
      name: "Settings",
    });
    winBus.refs.put(winRefs.freeze("desktop-uia", "settings"));

    const winResult = await dispatchComputeRoute(
      winBus,
    ...
    expect(uia.calls).toHaveLength(0);
    ...
    const linuxRefs = new RefAllocator();
    linuxRefs.alloc({
      stable: "desktop-atspi:settings:title",
      role: "text",
      name: "Settings",
    });
    linuxBus.refs.put(linuxRefs.freeze("desktop-atspi", "settings"));

    const linuxResult = await dispatchComputeRoute(
      linuxBus,
      { kind: "compute_assert", params: { text: "Settings" } },
      "linux",
    );
    ...
    expect(atspi.calls).toHaveLength(0);
  });
  ...
  it("does not change launch providers after subprocess failure", async () => {
    const winBus = createTransportBus();
    ...
    expect(winSubprocess.calls).toHaveLength(1);
    expect(uia.calls).toHaveLength(0);
    ...
    expect(linuxResult.ok).toBe(false);
    expect(linuxSubprocess.calls).toHaveLength(1);
    expect(atspi.calls).toHaveLength(0);
  });
  ...
  it("rejects contradictory UIA refs and CDP target evidence", async () => {
    const bus = createTransportBus();
    const cdp = new StubTransport(
      "cdp-browser",
      ["click"],
    ...
    if (!result.ok) {
      expect(result.error.reason).toContain(
        "browser target evidence conflicts with requested provider desktop-uia",
      );
    }
    expect(uia.calls).toHaveLength(0);
    expect(cdp.calls).toHaveLength(0);
  ...
  it("rejects contradictory AT-SPI refs and CDP target evidence", async () => {
    const bus = createTransportBus();
    const cdp = new StubTransport(
      "cdp-browser",
      ["click"],
    ...
    if (!result.ok) {
      expect(result.error.reason).toContain(
        "browser target evidence conflicts with requested provider desktop-atspi",
      );
    }
    expect(atspi.calls).toHaveLength(0);
    expect(cdp.calls).toHaveLength(0);
  ...
  it("preserves the selected provider failure without probing alternatives", async () => {
    const bus = createTransportBus();
    bus.register(
      new StubTransport(
        "cdp-browser",
    ...
    expect(result.ok).toBe(false);
    if (!result.ok) {
      expect(result.error.reason).toBe("desktop-uia unavailable");
      expect(result.error.minimum_capability).toBe(
        "desktop-uia.uia_screenshot",
      );
      expect(result.error.exit_code).toBe(69);
    }
  });

  it("does not fall from an explicit OS driver point action into visual", async () => {
    const bus = createTransportBus();
    const driver = new StubTransport(
      "cua-driver",
      ["cua_click"],
      failure("cua-driver", "cua_click"),
    );
    const visual = new StubTransport(
      "visual",
      ["visual_click"],
    ...
    const result = await dispatchComputeRoute(
      bus,
      {
        ...
        params: { x: 40, y: 80, via: "driver", observation },
      },
      "darwin",
    );
    ...
    expect(result).toMatchObject({
      ...
      error: {
        ...
        minimum_capability: "cua-driver.cua_click",
      },
    });
