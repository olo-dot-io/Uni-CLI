...
function renderLlmsTxt(
  ...
  const lines = [
    "# Uni-CLI",
    "",
    "Uni-CLI is the operations substrate for AI agents to discover, run, prove, and repair work across websites, logged-in browsers, desktop apps, local tools, external CLIs, MCP servers, and agent backends.",
    "",
    "## Catalog Snapshot",
    "",
    `- Sites: ${siteIndex.total_sites}`,
    ...
    "- On failure, read the v2 error envelope, open `error.adapter_path`, patch the named step, then run `unicli repair <site> <command>`.",
